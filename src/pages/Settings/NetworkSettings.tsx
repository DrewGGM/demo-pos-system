    ...
    try {
      ...
      if (restConfig) {
        setRestaurantConfig({
          ...
          kitchen_mode: restConfig.kitchen_mode || 'app',
          kitchen_printer_id: restConfig.kitchen_printer_id || null,
        });
      }
    } catch (error: any) {
    ...
    try {
      ...
      if (fullConfig) {
        await wailsConfigService.updateRestaurantConfig({
          ...
          kitchen_mode: restaurantConfig.kitchen_mode,
          kitchen_printer_id: restaurantConfig.kitchen_printer_id,
        });
          ...
          <Grid item xs={12}>
            <Card sx={{ border: '1px solid', borderColor: 'primary.main' }}>
              <CardContent>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                  Cómo se envían los pedidos a cocina y opciones de apps móviles
                </Typography>
                <Grid container spacing={2}>
                  <Grid item xs={12}>
                    <Typography variant="subtitle2" gutterBottom>Modo de Cocina</Typography>
                    <ToggleButtonGroup
                      value={restaurantConfig.kitchen_mode || 'app'}
                      exclusive
                      onChange={(_, val) => val && setRestaurantConfig({ ...restaurantConfig, kitchen_mode: val })}
                      size="small"
                      fullWidth
                    >
                      <ToggleButton value="app">
                        <Box sx={{ textAlign: 'center' }}>
                          <Typography variant="body2" fontWeight={600}>App Cocina</Typography>
                          <Typography variant="caption" color="text.secondary">Pedidos en pantalla</Typography>
                        </Box>
                      </ToggleButton>
                      <ToggleButton value="printer">
                        <Box sx={{ textAlign: 'center' }}>
                          <Typography variant="body2" fontWeight={600}>Impresora</Typography>
                          <Typography variant="caption" color="text.secondary">Ticket impreso</Typography>
                        </Box>
                      </ToggleButton>
                      <ToggleButton value="both">
                        <Box sx={{ textAlign: 'center' }}>
                          <Typography variant="body2" fontWeight={600}>Ambos</Typography>
                          <Typography variant="caption" color="text.secondary">App + Impresora</Typography>
                        </Box>
                      </ToggleButton>
                    </ToggleButtonGroup>
                    <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>
                      {restaurantConfig.kitchen_mode === 'app' && 'Los pedidos se envían a la app de cocina por WebSocket'}
                      {restaurantConfig.kitchen_mode === 'printer' && 'Los pedidos se imprimen como ticket de comanda en la impresora configurada'}
                      {restaurantConfig.kitchen_mode === 'both' && 'Los pedidos se envían a la app de cocina Y se imprimen como ticket'}
                    </Typography>
                  </Grid>
                  <Grid item xs={12}>
                    <Divider />
                  ...
                  <Grid item xs={12}>
                    <FormControlLabel
                      ...
                      label="Confirmaciones de cocina"
                    />
                    <Typography variant="caption" color="text.secondary" sx={{ display: 'block', ml: 7 }}>
                      La app de meseros muestra alerta si cocina no confirma recepción del pedido
                    </Typography>
                  </Grid>
                  <Grid item xs={12}>
