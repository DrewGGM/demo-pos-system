...
const Reports: React.FC = () => {
  ...
  const handleExportReport = async () => {
    try {
      ...
      const report = await wailsReportsService.getSalesReport(startDateStr, endDateStr);
      if (!report) {
      ...
      const rows: string[][] = [];
      rows.push(['Reporte de Ventas', `${startDateStr} a ${endDateStr}`]);
      rows.push([]);
      rows.push(['Métrica', 'Valor']);
      rows.push(['Total Ventas', String(report.total_sales || 0)]);
      rows.push(['Total IVA', String(report.total_tax || 0)]);
      rows.push(['Total Descuentos', String(report.total_discounts || 0)]);
      rows.push(['Número de Ventas', String(report.number_of_sales || 0)]);
      rows.push(['Venta Promedio', String(report.average_sale || 0)]);
      rows.push([]);

      if (report.payment_breakdown && Object.keys(report.payment_breakdown).length) {
        rows.push(['Método de Pago', 'Monto']);
        Object.entries(report.payment_breakdown).forEach(([method, amount]) => {
          rows.push([method, String(amount)]);
        });
        rows.push([]);
      }
      ...
      if (report.top_products?.length) {
        ...
        report.top_products.forEach((p: any) => {
          rows.push([p.product_name || '', String(p.quantity || 0), String(p.total_sales || 0)]);
        });
      }
      ...
      a.href = url;
      a.download = `reporte-${startDateStr}-a-${endDateStr}.csv`;
      a.click();
