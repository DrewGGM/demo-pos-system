...
const CashRegister: React.FC = () => {
  ...
  const handleDeleteMovement = async (movement: CashMovement) => {
    ...
    try {
      await wailsAuthService.deleteCashMovement(movement.id);
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                <Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                    <Chip
                      icon={isRegisterOpen ? <LockOpenIcon /> : <LockIcon />}
                      label={isRegisterOpen ? 'CAJA ABIERTA' : 'CAJA CERRADA'}
                      color={isRegisterOpen ? 'success' : 'error'}
                      sx={{ fontWeight: 700 }}
                    />
                  </Box>
                  <Typography variant="body2" color="text.secondary">
                    {registerStatus.opened_by} · {format(new Date(registerStatus.opened_at), "dd MMM yyyy, HH:mm", { locale: es })}
                  </Typography>
                  {registerStatus.closed_at && (
                    <Typography variant="body2" color="text.secondary">
                      Cierre: {format(new Date(registerStatus.closed_at), "dd MMM yyyy, HH:mm", { locale: es })}
                    </Typography>
                  )}
                </Box>
                <Box sx={{ textAlign: 'right', display: 'flex', gap: 3, alignItems: 'flex-end' }}>
                  <Box>
                  ...
                  <Box>
                    <Typography variant="caption" color="text.secondary">Ingresos en caja</Typography>
                    <Typography variant="h6" color="success.main">
                      +${(registerStatus.expected_amount - registerStatus.opening_amount).toLocaleString('es-CO')}
                    </Typography>
                  </Box>
                  <Box>
