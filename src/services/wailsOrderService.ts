...
function mapOrder(w: models.Order | null): Order {
  ...
  return {
    ...
    discount_type: (w as any).discount_type,
    discount_reason_id: (w as any).discount_reason_id,
    discount_reason: (w as any).discount_reason ? {
      id: (w as any).discount_reason.id as number,
      name: (w as any).discount_reason.name || '',
      dian_discount_id: (w as any).discount_reason.dian_discount_id || 0,
      dian_code: (w as any).discount_reason.dian_code || '',
      is_active: (w as any).discount_reason.is_active !== false,
      is_default: (w as any).discount_reason.is_default === true,
      allow_custom_text: (w as any).discount_reason.allow_custom_text === true,
      display_order: (w as any).discount_reason.display_order || 0,
    } : undefined,
    discount_reason_text: (w as any).discount_reason_text,
    total: w.total || 0,
    notes: w.notes || '',
    source: w.source || 'pos',
