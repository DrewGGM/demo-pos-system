...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = useCallback(async (username: string, password: string) => {
    try {
      ...
      if (register && register.id) {
        setCashRegisterId(register.id);
        toast.info(`Caja abierta: ${register.opening_amount}`, { toastId: 'cash-register-open' });
      } else {
        toast.warning('No hay caja abierta. Por favor abra la caja para realizar ventas.', { toastId: 'cash-register-warning' });
      }

      toast.success(`Bienvenido ${response.employee.name}`, { toastId: 'login-success' });
    } catch (error: any) {
      toast.error(error.message || 'Error al iniciar sesión', { toastId: 'login-error' });
      throw error;
    }
  }, []);
  ...
  const loginWithPIN = useCallback(async (pin: string) => {
    try {
      ...
      toast.error(error.message || 'PIN incorrecto', { toastId: 'login-error' });
      throw error;
    }
  }, []);
