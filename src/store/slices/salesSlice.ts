...
export const fetchSalesHistory = createAsyncThunk(
  'sales/fetchHistory',
  async ({ limit = 50, offset = 0, startDate, endDate, search }: {
    limit?: number; offset?: number; startDate?: string; endDate?: string; search?: string;
  }) => {
    return await wailsSalesService.getSalesHistory(limit, offset, startDate, endDate, search);
  }
);
...
const salesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchSalesHistory.fulfilled, (state, action) => {
      ...
      state.salesTotalCount = action.payload.total;
    });
