...
const PaymentDialog: React.FC<PaymentDialogProps> = ({
  ...
  // Prevents WebSocket and polling fallback from both calling handleBoldPaymentSuccess
  const boldSuccessHandledRef = useRef(false);
  ...
  const processBoldPayment = async (method: PaymentMethod, paymentAmount: number) => {
    try {
      ...
      // Pending payment record is needed so the webhook can find and update it
      ...
      boldSuccessHandledRef.current = false;
      setBoldIntegrationId(integrationId);
      setBoldPaymentWaiting(true);

      // Polling fallback (WebSocket is the primary mechanism)
      const maxPolls = 30;
      let pollCount = 0;
  ...
  const handleBoldPaymentSuccess = async (integrationId: string, pendingPayment?: models.BoldPendingPayment) => {
    if (boldSuccessHandledRef.current) {
      return;
    }
    boldSuccessHandledRef.current = true;
